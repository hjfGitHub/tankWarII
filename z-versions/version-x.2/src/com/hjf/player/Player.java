...
public class Player {
	...
	/*
	 * public static void main(String[] args) { Player player = new Player();
	 * //File file = new File("res/sound/开干1.mp3");
	 * //System.out.println(file.getName()); player.play(new
	 * File("res/sound/tankSounds/shoot.wav"), 100); }
	 */
		...
		try {
			...
			if (format.getEncoding() != AudioFormat.Encoding.PCM_SIGNED) {
				format = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, format.getSampleRate(), 16,
						format.getChannels(), format.getChannels() * 2, format.getSampleRate(), false); // big
																										// endian
